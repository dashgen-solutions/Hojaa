...
import { useMessagingConnection } from '@/contexts/MessagingConnectionContext';
import { WSMessage } from '@/hooks/useMessagingWebSocket';
import {
  getChannels,
  getChannelMessages,
  sendChannelMessage,
...
import UserProfileModal, { peerFromChannelMember, AvatarOrPlaceholder } from '@/components/messaging/UserProfileModal';
import type { PeerProfileData } from '@/components/messaging/UserProfileModal';
import CallTranscriptionsPanel from '@/components/messaging/CallTranscriptionsPanel';
...
export default function MessagesPage() {
  ...
  const [userStatuses, setUserStatuses] = useState<Record<string, string>>({}); // 'online' | 'offline' | 'in_call'
  ...
  /** Own status for sidebar (under username), like Slack */
  const [mySidebarStatus, setMySidebarStatus] = useState<{ text: string | null; emoji: string | null }>({
    text: null,
    emoji: null,
  });
  ...
  const dmPeer: PeerProfileData | null = useMemo(() => {
    if (!selectedChannel?.is_direct || !user) return null;
    const m = selectedChannel.members.find((x) => x.user_id !== user.id);
    if (m) return peerFromChannelMember(m);
    const ou = selectedChannel.other_user;
    if (!ou) return null;
    const online = selectedChannel.members.find((x) => x.user_id === ou.user_id)?.is_online;
    return {
      user_id: ou.user_id,
      username: ou.username,
      email: ou.email,
      avatar_url: ou.avatar_url,
      custom_status: ou.custom_status,
      status_emoji: ou.status_emoji,
      job_title: ou.job_title,
      org_role: ou.org_role,
      is_online: online,
    };
  ...
  useEffect(() => {
    if (!isAuthenticated) return;
    getMyStatus()
      .then((s) => setMySidebarStatus({ text: s.custom_status ?? null, emoji: s.status_emoji ?? null }))
      .catch(() => {});
  }, [isAuthenticated]);
  ...
  const handlePresence = useCallback(
    (msg: WSMessage) => {
      const userId = msg.user_id as string;
      const status = (msg.status as string) || 'offline';
  ...
  const handleStatusUpdate = useCallback(
    (msg: WSMessage) => {
      ...
      if (userId === user?.id) {
        setMySidebarStatus({ text: customStatus, emoji: statusEmoji });
      }
  ...
  const { registerPageHandlers, webrtcCall, sendTyping, connected } = useMessagingConnection();
  ...
  return (
    <div className="flex h-full overflow-hidden bg-white dark:bg-[#1e2024]">
          ...
          <>
            ...
            <div className="flex items-center justify-between px-5 py-2.5 border-b border-neutral-200 dark:border-[#383a3f] flex-shrink-0 bg-white dark:bg-[#1e2024]">
              <div className="flex items-center gap-3 min-w-0 flex-1">
                {selectedChannel.is_direct && dmPeer ? (
                  <button
                    type="button"
                    onClick={() => setShowPeerProfile(true)}
                    className="flex items-center gap-3 min-w-0 text-left rounded-md -mx-1 px-1 py-0.5 hover:bg-neutral-100 dark:hover:bg-[#383a3f] transition-colors"
                  >
                    <AvatarOrPlaceholder avatarUrl={dmPeer.avatar_url} username={dmPeer.username} size={36} rounded="lg" />
                    <div className="min-w-0">
                      ...
                      {(dmPeer.custom_status || dmPeer.status_emoji) ? (
                        <p className="text-[11px] text-neutral-500 dark:text-gray-500 truncate mt-0.5 max-w-[320px]">
                          {dmPeer.status_emoji && <span className="mr-1">{dmPeer.status_emoji}</span>}
                          {dmPeer.custom_status}
                        </p>
                      ) : null}
                    </div>
                  </button>
              ...
              <div className="flex items-center gap-1">
                ...
                <button
                  onClick={() => { setShowSearch(!showSearch); setShowPins(false); setThreadMessage(null); setShowTranscriptions(false); }}
                  className={`p-2 rounded-md transition-colors ${
                ...
                {/* Call transcriptions */}
                <button
                  onClick={() => { setShowTranscriptions((v) => !v); setShowPins(false); setShowSearch(false); setThreadMessage(null); }}
                  className={`p-2 rounded-md transition-colors ${
                    showTranscriptions ? 'bg-green-600/20 text-green-500' : 'text-neutral-400 dark:text-gray-400 hover:text-neutral-600 dark:hover:text-gray-200 hover:bg-neutral-100 dark:hover:bg-[#383a3f]'
                  }`}
      ...
      {/* Call UI is rendered globally by MessagingConnectionProvider */}

      {showPeerProfile && dmPeer && (
        <UserProfileModal peer={dmPeer} onClose={() => setShowPeerProfile(false)} />
      )}

      {/* Call transcriptions panel (right side) */}
      {showTranscriptions && selectedChannelId && selectedChannel && (
        <CallTranscriptionsPanel
          channelId={selectedChannelId}
          channelName={selectedChannel.is_direct ? (selectedChannel.other_user?.username || 'Direct Message') : (selectedChannel.name || 'Channel')}
          onClose={() => setShowTranscriptions(false)}
        />
      )}
